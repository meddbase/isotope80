...
Inheritance [Object](https://docs.microsoft.com/en-us/dotnet/api/system.object) → [ValueType](https://docs.microsoft.com/en-us/dotnet/api/system.valuetype) → [IsotopeAsync&lt;Env, A&gt;](./isotope80.isotopeasync-2.md)

## Constructors

### **IsotopeAsync(Func&lt;Env, IsotopeState, ValueTask&lt;IsotopeState&lt;A&gt;&gt;&gt;)**

Ctor

```csharp
IsotopeAsync(Func<Env, IsotopeState, ValueTask<IsotopeState<A>>> thunk)
```

#### Parameters

`thunk` Func&lt;Env, IsotopeState, ValueTask&lt;IsotopeState&lt;A&gt;&gt;&gt;<br>
Thunk
...
```csharp
ValueTask<ValueTuple<IsotopeState, A>> Run(Env env, IsotopeSettings settings)
```
...
```csharp
ValueTask<ValueTuple<IsotopeState, A>> RunAndThrowOnError(Env env, IsotopeSettings settings)
```
...
```csharp
IsotopeAsync<Env, A> Pure(A value)
```
...
[IsotopeAsync&lt;Env, A&gt;](./isotope80.isotopeasync-2.md)<br>
...
```csharp
IsotopeAsync<Env, A> Fail(Error error)
```
...
```csharp
IsotopeAsync<Env, B> Bind<B>(Func<A, Isotope<B>> f)
```
...
```csharp
IsotopeAsync<Env, B> Bind<B>(Func<A, Isotope<Env, B>> f)
```
...
```csharp
IsotopeAsync<Env, B> SelectMany<B>(Func<A, Isotope<B>> f)
```
...
```csharp
IsotopeAsync<Env, B> SelectMany<B>(Func<A, Isotope<Env, B>> f)
```
...
```csharp
IsotopeAsync<Env, C> SelectMany<B, C>(Func<A, Isotope<B>> bind, Func<A, B, C> project)
```
...
```csharp
IsotopeAsync<Env, C> SelectMany<B, C>(Func<A, Isotope<Env, B>> bind, Func<A, B, C> project)
```
...
```csharp
IsotopeAsync<Env, B> Map<B>(Func<A, B> f)
```
...
```csharp
IsotopeAsync<Env, B> Select<B>(Func<A, B> f)
```
...
```csharp
IsotopeAsync<Env, A> MapFail(Func<Seq<Error>, Seq<Error>> f)
```
...
```csharp
IsotopeAsync<Env, B> BiMap<B>(Func<A, B> Succ, Func<Seq<Error>, Seq<Error>> Fail)
```
...
```csharp
IsotopeAsync<Env, A> MapFail(Func<Seq<Error>, Error> f)
```
...
```csharp
IsotopeAsync<Env, B> BiMap<B>(Func<A, B> Succ, Func<Seq<Error>, Error> Fail)
```
